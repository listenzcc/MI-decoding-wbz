"""
File: decode-fbcnet-cvruns.py
Author: Chuncheng Zhang
Date: 2025-11-07
Copyright & Email: chuncheng.zhang@ia.ac.cn

Purpose:
    Decode with FBCNet, cv across runs.
...
OUTPUT_DIR = Path(f'./data/exp_record/results/fbcnet-cvruns/{SUBJECT}')
...
for low_freq, high_freq in tqdm(freq_bands, 'Filtering'):
    ...
    for i in range(X.shape[0]):
        # 创建Epochs对象进行滤波
        _epochs = mne.EpochsArray(X[i:i+1], info, tmin=tmin)
